<script lang="ts">
	...
	export let open: boolean,
		existing: { level: 'image' | 'regions'; config: LabelConfig } | undefined = undefined;
	const emptyState = {
	...
	$: state = existing
		? {
				...
				options: existing.config.options?.map((o) => o.displayName || o.name) || ([] as string[])
			}
		: emptyState;
	$: valid = state.name && (state.properties.indexOf('freeform') > -1 || state.options.length > 0);
