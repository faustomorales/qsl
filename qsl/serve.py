...
def get_session(existing_session=None):
...
def get_project(
    ...
    with get_session(existing_session) as session:
        project = (
        ...
        for config, option in configs:
            subgroup = getattr(
                getattr(labeling, config.level.name), config.label_type.name
            )
...
def create_project(
    ...
    with get_session(existing_session) as session:
        ...
        if project.labelingConfiguration is not None:
            ...
            session.commit()
        return get_project(project_id=project_orm.id, existing_session=session)
...
def update_project(
    ...
    with get_session() as session:
        ...
        session.commit()
        return get_project(project_id=project_id, existing_session=session)
...
def create_images(
    ...
    with get_session(existing_session) as session:
        project = get_project(project_id=project_id, existing_session=session)
        now = time.time()
        assert (
...
def list_images(
    ...
    with get_session() as session:
        ...
        status = sa.sql.expression.case(
            [
                (
                    sa.func.max(sa.sql.cast(current_user_labels.ignored, sa.Integer))
                    == 1,
                    "ignored",
                ),
...
def get_file(
    ...
    with get_session() as session:
        ...
        if filepath.startswith("http://") or filepath.startswith("https://"):
            return fastapi.responses.RedirectResponse(
                filepath, headers={"Cache-Control": "max-age=60"}
            )
...
def set_labels(
    ...
    with get_session(existing_session) as session:
        ...
        if labels.ignored:
            ...
            config = get_project(
                project_id=project_id, existing_session=session
...
def export_project(
    ...
    with get_session(existing_session) as session:
        label_mapping: typing.Dict[
            int,
            typing.Dict[int, typing.Tuple[web.LabelGroup, typing.Dict[int, web.Box]]],
        ] = {}
        ...
        for query_user_id in [
            u.id for u in list_users(existing_session=session, user=user)
        ] + [None]:
            for (
                image_id,
                image_filepath,
                ...
                if (query_user_id is not None) and (image_id not in label_mapping):
                    ...
                    label_mapping[image_id][user_id] = (labels, box_lookup)
                elif (query_user_id is None) and (
                    image_id not in default_label_mapping
                ):
...
def list_users(
    user: web.User = fastapi.Depends(get_current_user), existing_session=None
...
async def auth(
    ...
    with get_session() as session:
        if oauth is None:
            ...
            resp = await oauth.client.provider.get(
                oauth.userinfo_query_key, token=token
            )
...
def default_startup(app: fastapi.FastAPI, engine=None):
    ...
    def startup():
        ...
        user = next(
            (user for user in list_users() if user.name == initial_user),
            None,
        )
        if user is None:
            user = create_user(
...
def launch_simple_app(host: str, port: int, project: web.Project):
    ...
    with session_maker() as session:
        orm.BaseModel.metadata.create_all(engine)
        project_id = create_project(project=project, existing_session=session).id
    ...
    app = build_app()
    app.on_event("startup")(default_startup(app=app, engine=engine))
    uvicorn.run(
        app,
        host=host,
        port=port,
    ...
    with session_maker() as session:
        return export_project(
            user=user, project_id=project_id, existing_session=session
        )
