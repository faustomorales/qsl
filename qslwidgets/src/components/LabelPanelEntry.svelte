<script lang="ts">
	...
	$: keydown = (event: KeyboardEvent) => {
		if (
			...
			simulateClick(
				element.querySelector(`.input-target[data-index='${idx}']`) as HTMLElement
			).then(() => focus(element));
...
<div bind:this={element} class="label-panel-entry">
	...
	<div class="label-panel-entry-values" style="flex-direction: {config.layout || 'column'};">
		{#if options}
		...
		{#if config.freeform}
			{#if options || config.freeformtag === 'input'}
				<input
					...
					on:keydown={(event) =>
						event.key === 'Enter' ? freeformEffects.submit(event) : freeformEffects.modify()}
					class="input-target"
				/>
			{:else}
				<textarea
